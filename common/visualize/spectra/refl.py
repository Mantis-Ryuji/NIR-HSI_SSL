...
def plot_refl_spectra(
    wavenumber_nm: np.ndarray,
    refl_nc: np.ndarray,
    refl_snv_nc: np.ndarray,
    cmap: ListedColormap,
    *,
    ...
    """
    反射率スペクトルとSNV後スペクトルを、波数(cm^-1)軸で並べて可視化する。

    Parameters
    ----------
    wavenumber_nm : np.ndarray, shape (C,)
        波長[nm]。0より大きい値のみを想定。
    refl_nc : np.ndarray, shape (N, C)
        反射率スペクトル（例: 各クラスタの平均スペクトル）。
    refl_snv_nc : np.ndarray, shape (N, C)
        SNV後の反射率スペクトル。
    ...
    refl_nc = np.asarray(refl_nc)
    refl_snv_nc = np.asarray(refl_snv_nc)

    if wavenumber_nm.ndim != 1:
        raise ValueError(f"wavenumber_nm must be 1D (C,), got shape={wavenumber_nm.shape}")
    if refl_nc.ndim != 2:
        raise ValueError(f"refl_nc must be 2D (N,C), got shape={refl_nc.shape}")
    if refl_snv_nc.ndim != 2:
        raise ValueError(f"refl_snv_nc must be 2D (N,C), got shape={refl_snv_nc.shape}")

    N, C = refl_nc.shape
    if refl_snv_nc.shape != (N, C):
        raise ValueError(
            f"refl_snv_nc must have same shape as refl_nc: expected {(N, C)}, got {refl_snv_nc.shape}"
        )
    ...
    ax12.set_ylabel("SNV(Reflectance)")
    ...
    for idx in range(N):
        ...
        ax11.plot(wavenumber_cm, refl_nc[idx, :], c=color, lw=1.0, label=label)
        ax12.plot(wavenumber_cm, refl_snv_nc[idx, :], c=color, lw=1.0, label=label)
